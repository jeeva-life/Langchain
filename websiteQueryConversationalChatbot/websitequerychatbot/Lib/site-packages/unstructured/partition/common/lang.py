...
def _get_iso639_language_object(lang: str) -> Optional[iso639.Language]:
    try:
        language = iso639.languages.get(part1=language_code)  # Try ISO 639-1
        if not language:
            language = iso639.languages.get(part2b=language_code)  # Try ISO 639-2
        if not language:
            logger.warning(f"{language_code} is not a valid standard language code.")
        return language
    except Exception as e:
        logger.warning(f"Error with language code '{language_code}': {e}")
        return None
...
def detect_languages(
    ...
    # Default to English if text contains only ASCII characters and is short
    ...
    # Handle user-provided languages
    if languages and "auto" not in languages:
        for lang in languages:
            try:
                ...
                logger.warning(f"Invalid language code '{lang}': {e}")
                continue
    ...
    # Handle auto language detection
        ...
        # Normalize and handle detected languages
        for langobj in langdetect_result:
            try:
                if str(langobj.lang).startswith("zh"):  # Handle Chinese variants
                ...
                logger.warning(f"Error processing detected language '{langobj.lang}': {e}")
                continue
